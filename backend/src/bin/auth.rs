...
use multiplayer_cards::{Claims, Services};
...
async fn guest_login_handler(_event: Request, services: &Services) -> Result<Response<Body>, Error> {
...
async fn refresh_token_handler(event: Request, services: &Services) -> Result<Response<Body>, Error> {
    ...
    let Some(token_str) = (match auth_header {
        ...
        return Ok((StatusCode::UNAUTHORIZED, "Missing authorization header").into_response().await)
    };

    // todo these possibly aren't getting deleted
    let Ok(uuid) = services.delete::<RefreshToken>(&token_str.to_string(), None).await else {
        return Ok((StatusCode::UNAUTHORIZED, "Unauthorized").into_response().await);
    };
    let access_token = generate_jwt(&uuid).await?;
    let new_refresh_token = generate_refresh_token();
    let _ = services.put::<RefreshToken>(&new_refresh_token, &uuid).await?;
...
async fn main() -> Result<(), Error> {
    ...
    run(service_fn(async |event: Request| {
        ...
        match route_key.as_str() {
            "/guest" => guest_login_handler(event, &services).await,
            "/refresh" => refresh_token_handler(event, &services).await,
